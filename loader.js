...
function load_module(link,type,div,param) {
	var cont = document.getElementById(div);
	var loading = "<div class=load_module>Loading...</div>";
	cont.innerHTML = loading;
	var http = createRequestObject();
...
if( http ) 
{
	...
	http.open('get','modules/'+link+'/'+type+'.php'+param, true);
	http.onreadystatechange = function () 
	{
		if(http.readyState == 4) {
		  if (http.status == 200) {
...
function load_block(link,type,div,param) {
	var cont = document.getElementById(div);
	var loading = "<div class=load_module>Loading...</div>";
	cont.innerHTML = loading;
	var http = createRequestObject();
...
if( http ) 
{
	...
	http.open('get','block/'+link+'/'+type+'.php'+param, true);
	http.onreadystatechange = function () 
	{
		if(http.readyState == 4) {
		  if (http.status == 200) {
